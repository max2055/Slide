...
export async function evaluateOperationalRule(ref: Ref, rule: Rule, seconds: number, macros: Record<string, number> = {}) {
  ...
  if (!dbConnection.isConnected()) return rule.metric_name.includes('.') ? unavailable : { ...unavailable, handled: false };
  const actor = actorFor(ref);
  try {
    let effective;
    try { effective = await policyService.effective(actor, ref); }
    catch (e) { if (e instanceof PolicyError && e.code === 'POLICY_NOT_FOUND') return { ...unavailable, handled: rule.metric_name.includes('.') }; throw e; }
...
export async function migrateOperationalChecks(id: number, checks: Array<{ name: string; status: string; score: number; message?: string }>) {
  if (!dbConnection.isConnected()) return checks;
  const ref: Ref = { type: 'instance', id }, actor = actorFor(ref);
  try { await policyService.effective(actor, ref); }
  catch (e) { if (e instanceof PolicyError && e.code === 'POLICY_NOT_FOUND') return checks;
    return checks.map(c => c.name === '连接状态' ? c : { ...c, status: 'unknown', score: 0, message: '标准指标不可读取，不能判为健康' }); }
