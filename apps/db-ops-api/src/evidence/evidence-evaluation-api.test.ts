...
it('routes evaluation and metadata writes through the authenticated actor and masks internal failures', async () => {
  ...
  const service = { evaluate: vi.fn(async () => ({ rulesVersion: 0 })), rules: vi.fn(async () => ({ version: 0 })), updateRules: vi.fn(async () => { throw new Error('RULES_VERSION_CONFLICT'); }), recordDecision: vi.fn(async () => ({ id: 'd' })), decision: vi.fn(async () => null), decisions: vi.fn(async () => ({ items: [], truncated: false, gaps: [] })), recovery: vi.fn(async () => ({ status: 'unknown' })) };
