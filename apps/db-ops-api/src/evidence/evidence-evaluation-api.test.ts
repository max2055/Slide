...
it('routes evaluation and metadata writes through the authenticated actor and masks internal failures', async () => {
  ...
  const service = { evaluate: vi.fn(async () => ({ rulesVersion: 0 })), rules: vi.fn(async () => ({ version: 0 })), updateRules: vi.fn(async () => { throw new Error('RULES_VERSION_CONFLICT'); }), recordDecision: vi.fn(async () => ({ id: 'd' })), decision: vi.fn(async () => null), decisions: vi.fn(async () => ({ items: [], truncated: false, gaps: [] })), recovery: vi.fn(async () => ({ status: 'unknown' })), recoveryPolicy: vi.fn(async () => ({ enabled: false, version: 0 })), updateRecoveryPolicy: vi.fn(async () => { throw new Error('RECOVERY_POLICY_VERSION_CONFLICT'); }) };
  ...
  expect((await app.inject('/api/resources/server/1/decisions?limit=10')).statusCode).toBe(200);
  expect(service.decisions).toHaveBeenCalledWith(actor, { type: 'server', id: 1 }, 10);
  expect((await app.inject('/api/resources/server/1/recovery-policy')).statusCode).toBe(200);
  expect((await app.inject({ method: 'PUT', url: '/api/resources/server/1/recovery-policy', payload: { expectedVersion: 1 } })).statusCode).toBe(409);
