...
export function publicNotificationDto(channel: Record<string, unknown>) {
  ...
  const { secret, token, password, apiKey, webhook_url, ...config } = rawConfig;
  ...
  return { ...channel, config: { ...config, endpoint, hasCredential: Boolean(secret || token || password || apiKey) } };
}
