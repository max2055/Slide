...
export class GitLabSourceConnector {
  constructor(private readonly allowedOrigins: readonly string[], private readonly request: typeof fetch = fetch) {}
  ...
  async fetchFiles(config: GitLabSourceConfig): Promise<SourceFile[]> {
    ...
    const read = async (url: string, max: number) => {
      try {
        const response = await this.request(url, { headers: { 'PRIVATE-TOKEN': config.token }, redirect: 'error', signal: AbortSignal.timeout(15_000) });
      ...
      } catch { throw new Error('SOURCE_UPSTREAM_UNAVAILABLE'); }
    };
    ...
    for (const path of paths) {
      ...
      assertSourceContent(content); files.push({ path, content });
    }
    return files;
  }
}
