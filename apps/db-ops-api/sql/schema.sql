...
CREATE TABLE IF NOT EXISTS `metrics_history` (
  ...
  `active_transactions` INT DEFAULT NULL COMMENT '活跃事务',
  `slow_queries` INT DEFAULT NULL COMMENT '慢查询查询',
  `buffer_pool_hit_rate` DECIMAL(5,2) DEFAULT NULL COMMENT '缓冲池命中率 %',
  `threads_running` INT DEFAULT NULL COMMENT '线程运行',
  `threads_connected` INT DEFAULT NULL COMMENT '线程已连接',
  `bytes_received` BIGINT DEFAULT NULL COMMENT '字节接收',
  `bytes_sent` BIGINT DEFAULT NULL COMMENT '字节发送',
  `queries_total` BIGINT DEFAULT NULL COMMENT '查询总计',
  `commits_total` BIGINT DEFAULT NULL COMMENT '提交事务总计',
  `rollbacks_total` BIGINT DEFAULT NULL COMMENT '回滚事务总计',
...
CREATE TABLE IF NOT EXISTS `slow_queries` (
  ...
  `tables_accessed` JSON DEFAULT NULL COMMENT '访问的表名列表 ["orders","users"]',
...
CREATE TABLE IF NOT EXISTS `alert_events` (
  ...
  `title` VARCHAR(200) NOT NULL,
  `description` TEXT DEFAULT NULL,
  `status` ENUM('open', 'investigating', 'handled', 'resolved', 'closed', 'reviewed') NOT NULL DEFAULT 'open',
...
CREATE TABLE IF NOT EXISTS `approval_requests` (
  ...
  `rollback_info` JSON DEFAULT NULL COMMENT '回滚信息 {sql, executed_at, success, notes}',
  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  INDEX `idx_status` (`status`),
...
CREATE TABLE IF NOT EXISTS `sql_execution_history` (
  ...
  `error_message` TEXT DEFAULT NULL,
  `ip_address` VARCHAR(45) DEFAULT NULL,
  `approval_request_id` BIGINT UNSIGNED DEFAULT NULL COMMENT '审批请求 ID，关联 approval_requests.id',
...
CREATE TABLE IF NOT EXISTS `network_devices` (
  ...
  `vendor` ENUM('huawei','cisco') NOT NULL DEFAULT 'huawei' COMMENT '设备厂商',
...
CREATE TABLE IF NOT EXISTS `network_device_credentials` (
  ...
  `protocol` ENUM('snmpv2c','snmpv3','ssh') NOT NULL COMMENT '凭据协议',
  ...
  `community_encrypted` TEXT DEFAULT NULL COMMENT '加密存储的 SNMPv2c community',
