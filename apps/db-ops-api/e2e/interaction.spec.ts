...
test.describe("Instance detail interaction", () => {
  test("navigate to instance and traverse sub-tabs", async ({ page }) => {
    ...
    // Find sub-tabs — retry with wait since SPA navigation may delay rendering
    let tabs = page.locator("button.tab");
    let tabCount = await tabs.count();
    for (let attempt = 0; attempt < 5 && tabCount === 0; attempt++) {
    ...
    if (tabCount === 0) {
      ...
      await page.waitForTimeout(3000);
      tabCount = await page.locator("button.tab").count();
      console.log(`  After fallback, sub-tabs: ${tabCount}`);
    }

    // Click each sub-tab by index (tabs are always in same order)
    const subTabs = ["overview", "metrics", "topsql", "trend", "sessions", "capacity", "schema", "indexes", "sqlaudit", "logs"];
    for (let i = 0; i < subTabs.length && i < tabCount; i++) {
      const sub = subTabs[i];
      cur = `instance-${sub}`;
