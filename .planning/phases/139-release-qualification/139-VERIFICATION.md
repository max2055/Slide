...
|---|---|---|
| Backend unit suite | PASS | `pnpm --filter slide-api test`: 87 files, 966 tests passed, including Microsoft OAuth refresh-token rotation persistence and Feishu signed-webhook payload/encrypted-secret handling. |
...
| HI-09 | Partial | Scheduled report is generated, persisted, downloaded, and records a failed notification; SMTP email implementation encrypts password and OAuth refresh-token credentials at rest, obtains Microsoft SMTP OAuth2 access tokens, atomically persists a Microsoft-rotated refresh token as ciphertext, and has local contract coverage. Feishu custom-webhook signing is locally verified: it encrypts the signing secret at rest and sends the documented timestamp/HMAC fields in the JSON body rather than the URL. The authorized Hotmail qualification is blocked before SMTP authentication by the execution environment's reserved-address egress route. A real successful external delivery remains unverified. |
