...
| Feishu external delivery | BLOCKED | A disabled, encrypted Feishu test channel was configured with the authorized body-signing secret and tested once through the protected endpoint. The execution environment resolves `open.feishu.cn` to reserved address `198.18.3.98`; outbound policy correctly rejected it as `PRIVATE_ADDRESS`, so no HTTP request reached Feishu and no real delivery can be claimed. Migration 047 prevents an enabled channel from retrying alerts that predate its activation boundary. |
...
| HI-09 | Partial | Scheduled report is generated, persisted, downloaded, and records a failed notification; SMTP email implementation encrypts password and OAuth refresh-token credentials at rest, obtains Microsoft SMTP OAuth2 access tokens, atomically persists a Microsoft-rotated refresh token as ciphertext, and has local contract coverage. Feishu custom-webhook signing is locally verified: it encrypts the signing secret at rest and sends the documented timestamp/HMAC fields in the JSON body rather than the URL. Managed Playwright creates a disabled Feishu channel through Settings and confirms its subsequent protected API response contains neither the signing secret nor the webhook path. A migration-backed activation boundary now prevents a newly enabled channel from replaying pre-existing unread alerts, including queued deliveries checked at worker execution. The authorized Hotmail run is blocked before SMTP authentication by the reserved-address egress route; the authorized Feishu run is similarly rejected before HTTP dispatch because `open.feishu.cn` resolves to reserved `198.18.3.98` and policy returns `PRIVATE_ADDRESS`. A real successful external delivery remains unverified. |
...
| ME-07 | Partial | Managed browser approval flow proves an approved SQL write executes once under concurrent review, then reads back the persisted operation as `succeeded` and its immutable CREATED → approval → execution event timeline. A managed recovery scenario submits a write without executing it, cancels the waiting operation, creates retry attempt 2, and reads back both the new queued operation and the approval record now attached to that current attempt. Browser button interaction remains unqualified. |
