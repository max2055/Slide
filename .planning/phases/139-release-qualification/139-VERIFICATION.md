...
| Managed browser qualification | PASS | `QUALIFICATION_ADMIN_PASSWORD=… PLAYWRIGHT_MANAGED_ENV=1 pnpm --filter slide-frontend exec playwright test e2e/release-security.spec.ts e2e/release-critical-paths.spec.ts e2e/agent-capabilities.spec.ts --workers=1`: 21 passed on 2026-07-19. The managed launcher resets only `db_ops_ai_qualification`, applies the current 49-migration ledger, then seeds its fixtures; this prevents a stale qualification ledger from being mistaken for a migration compatibility failure. The suite now creates an isolated MySQL instance through the API, uses the browser's existing connection-test/reload flow, reads native realtime metrics, renders them in the detail workspace, and verifies a browser refresh. |
...
| HI-06 | Partial | Real MySQL server and instance metric-to-threshold-alert workflows passed. Managed browser coverage creates an isolated MySQL instance, drives the connection-test/reload path, reads native realtime metrics, renders metric cards, and manually refreshes them; it also starts a server-alert RCA from the Alerts workspace without an `instance_id` and reads its completed state. One browser scenario that joins freshly collected metrics to a threshold-created alert and its RCA remains unqualified. |
| HI-07 | Partial | The server evaluator consumed canonical `cpu_usage`; the instance evaluator consumed a persisted `metrics_data` metric and created the expected alerts. The browser now reads and renders native MySQL metrics after the actual connection-test/reload path. Full collector-to-registry scheduling remains unqualified. |
| HI-08 | Partial | Both server and instance alerts created and read back persisted `alert_rca` analyses with the correct resource subject and related alert. A real database-configured DeepSeek run completed both RCA analyses through `slide_complete_analysis`; managed browser coverage starts and observes completion for a server-alert RCA. A single browser run tying freshly collected metrics to the resulting threshold alert and RCA remains unqualified. |
