...
| HI-09 | Partial | Scheduled report is generated, persisted, downloaded, and records a failed notification; SMTP email implementation encrypts password and OAuth refresh-token credentials at rest, obtains Microsoft SMTP OAuth2 access tokens, atomically persists a Microsoft-rotated refresh token as ciphertext, and has local contract coverage. Feishu custom-webhook signing is locally verified: it encrypts the signing secret at rest and sends the documented timestamp/HMAC fields in the JSON body rather than the URL. The authorized Hotmail run is blocked before SMTP authentication by the reserved-address egress route; the authorized Feishu run is similarly rejected before HTTP dispatch because `open.feishu.cn` resolves to reserved `198.18.3.98` and policy returns `PRIVATE_ADDRESS`. The disabled channel was not enabled for a bulk retry of historical alerts. A real successful external delivery remains unverified. |
