...
| 门槛 | 当前缺口与下一步 |
| --- | --- |
| SQL/Host 生命周期证据 | 生产资产访问已接线，但没有各引擎统计重置、Linux boot/interface/device 生命周期发现器；缺证据的计数继续未知。不得把 trial evidence 改名后视为完成。 |
...
| 全部消费入口 | V2 consumer runtime 已受控；旧资源 summary/history、`alert-evaluator.ts`、baseline/评分、诊断和旧 Agent 工具还需以已确认的指标语义映射接入统一选择。禁止同名强制映射或补造 V2 历史。 |
