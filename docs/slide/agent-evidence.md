...
Sensitive-pattern matches, unscannable JSON/YAML and text files over 512 KiB are retained unchanged. They produce signed `warnings` (path/reason only), shown as “提示（文件已保留）”; warnings neither block synchronization/reads nor mark a snapshot partial. Content scanning is advisory for the operator-controlled internal repository. If model sharing is enabled, flagged content is readable by the Agent too.

Unsupported paths, binary content, symlinks and individual files exceeding the entire 32 MiB snapshot budget are still skipped. Actual omissions produce `completeness=partial` and signed `skippedFiles` paths/reasons. Excluded directories are reported once, not recursively enumerated. If no supported text files remain, synchronization fails. Files outside configured directory prefixes are out of scope, not omissions. Git access-token protection remains separate: the token supplied for synchronization is never stored or exposed to the model.

Selected scanning is bounded to 20,000 entries (including directories) and 32 MiB of accepted content. There is no separate 512 KiB file rejection: one text file may consume the entire snapshot budget. The signed manifest is at most 2 MiB. Git commands and file selection share a 120-second deadline capped by credential expiry. The checkout is temporary and removed on success or failure. These limits bound snapshot processing, not the size of the downloaded Git pack/checkout: keep adequate free space in the persistent volume.

Manifest HMAC, internal tree digest, per-file digests, path restrictions and symlink checks remain mandatory. Deployment comparison is separate from integrity verification. Repository metadata, completeness, omissions and advisory warnings are signed on disk; the response's `verification` field is computed from current deployment configuration and is not part of that signature. Source snippets are at most 200 lines/16 KiB; source calls share a 30/minute budget and two concurrent readers per API process. Source envelopes include a warning count; region reads include warnings for that file, without repeating the whole manifest on every read.
