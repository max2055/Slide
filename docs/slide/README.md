...
Slide v0.10 是一个 AI 原生的基础运维助手，覆盖数据库、Kylin/RHEL/CentOS 服务器和华为 VRP 网络设备。平台提供跨资源监控、智能告警、关系影响分析、只读诊断和审计能力；网络设备支持 SNMPv3 观测与加密 SSH 配置备份，不提供配置下发或恢复。华为厂商级系统指标需经过目标型号/VRP 固件 UAT 后启用，未确认的 OID 明确返回 unsupported。
