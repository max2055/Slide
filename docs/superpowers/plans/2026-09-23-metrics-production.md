...
## Execution contract v6 — shared resource observation source control

Continue the existing consumer inventory without inventing legacy-to-canonical equivalence. Put the resource overview, metrics summary, diagnosis and Agent diagnosis legacy observation reader behind the resource-level formal source state. Delegate to old tables only for a uniformly applied legacy resource; return no legacy observations for pending/mixed/V2 and propagate source-state failure so existing gap handling reports uncertainty. Keep formal V2 diagnostic evidence in the existing semantic consumer response. This closes one shared bypass but leaves baseline, reports, direct history APIs and canonical UI migration open.
