...
export class NetworkDeviceDetail extends LitElement {
  ...
  render() {
    ...
    ${id === null ? html`<div class="error">网络设备 ID 无效</div>` : this.loading ? html`<div class="loading" role="status">正在加载设备数据…</div>` : this.error ? html`<div class="error" role="alert">${this.error}<br><button class="btn" type="button" @click=${this.loadContext}>重试</button></div>` : this.device ? html`<div class="page">
      ...
      ${this.activeTab === "collection" ? html`<metric-configuration resourceType="network_device" .resourceId=${this.validId()}></metric-configuration>` : this.activeTab === "overview" ? this.renderSummary() : this.activeTab === "interfaces" ? this.renderInterfaces() : this.activeTab === "relations" ? this.renderRelations() : html`${this.renderBackupSchedule()}${this.renderBackups()}`}
    </div>` : nothing}
