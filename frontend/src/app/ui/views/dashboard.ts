...
interface Capacity { current_total_gb: number | null; trend: Array<{ time: string; total_size_gb: number | null; instance_count?: number; observed_at?: string }> }
...
export class DashboardPage extends LitElement {
  ...
  private readonly onPopState = () => { this.readUrl(); this.normalizeSelection(); void this.loadTrend(); };
  ...
  private normalizeSelection() {
    if (this.selectedRisk && !this._visibleResourceItems().some(item => resourceKey(item.resource) === this.selectedRisk && isRisk(item))) { this.selectedRisk = ''; this.saveUrl(); }
    const databases = this._visibleResourceItems().filter(item => item.resource.type === 'instance');
    if (!databases.some(item => item.resource.id === this.selectedInstanceId)) this.selectedInstanceId = null;
  }
  ...
  private renderKpis(items: OverviewItem[]) {
    ...
    const variants = [undefined, (['server', 'network_device'].includes(this.resourceScope) ? s.unavailable : s.abnormal) > 0 ? s.unavailable > 0 ? 'danger' : 'warn' : undefined, !s.alertsUnavailable && s.alerts > 0 ? 'warn' : undefined, s.collection > 0 && (this.resourceScope === 'all' || !totalIncomplete) ? 'warn' : undefined];
    return html`<div class="dashboard__stat-cards">${cards.map((card, index) => html`<stat-card .variant=${variants[index]} aria-pressed=${String(this.detailFilter === card.filter && !this.detailType)} tabindex="0" role="button" aria-label=${`${card.label} ${card.value}`} .label=${card.label} .value=${card.value} .hint=${card.hint} @click=${() => this.selectDetail(card.filter as DetailFilter)} @keydown=${(event: KeyboardEvent) => { if (['Enter', ' '].includes(event.key)) { event.preventDefault(); this.selectDetail(card.filter as DetailFilter); } }}></stat-card>`)}</div>`;
  }
  private renderRiskQueue(items: OverviewItem[]) {
    const risks = items.filter(isRisk).sort(compareRisk);
    return html`<app-card class="risk-panel"><div slot="header" class="toolbar"><h2>${o('riskQueue')} · ${risks.length}</h2><button class="btn-ghost" @click=${() => { this.selectDetail('risk'); this.renderRoot.querySelector('#resource-details')?.scrollIntoView({ block: 'start' }); }}>${o('allRisks')}</button></div>
      <small>${o('topFive')}</small>${risks.slice(0, 5).map(item => html`<article class=${`risk-row${this.selectedRisk === resourceKey(item.resource) ? ' selected' : ''}`} data-resource=${resourceKey(item.resource)}>
        <div class="risk-identity"><span aria-hidden="true">${this.icon(item.resource.type)}</span><a class="resource-name" href=${this.detailHref(item.resource)} @click=${this.savePosition}>${this.label(item)}</a>${this.renderState(item)}</div>
        ${this.renderActions(item)}
        <div class="metadata risk-observation">${this.renderFreshness(item)}${freshness(item) === 'fresh' && collectionRisk(item) ? html`<span>${o('qualityGap')}</span>` : nothing}${item.unresolvedAlerts || item.gaps.includes('ALERTS_UNAVAILABLE') ? html`<span>${o('alerts')}: ${this.alertCount(item)}</span>` : nothing}<time title=${item.observedAt ?? ''}>${o('observed')}: ${this.time(item.observedAt)}</time><button class="btn-ghost relation-select" aria-pressed=${String(this.selectedRisk === resourceKey(item.resource))} aria-controls="risk-relations" @click=${() => { this.selectedRisk = this.selectedRisk === resourceKey(item.resource) ? '' : resourceKey(item.resource); this.saveUrl(); }}>${o('relations')} ${item.gaps.includes('RELATIONS_UNAVAILABLE') ? '—' : item.impactScope.length}</button></div>
        </article>`)}
  ...
  private renderHealth(items: OverviewItem[]) {
    ...
    return html`<app-card class="status-panel"><h2 slot="header">${o('health')}</h2>${resourceTypes.filter(type => this.resourceScope === 'all' || this.resourceScope === type).map(type => {
      const subset = items.filter(item => item.resource.type === type);
      const unavailable = this.resourceOverview?.unavailableTypes?.includes(type);
      const canView = permissionMatches(this.permissions, type === 'instance' ? 'instance:view' : type === 'server' ? 'servers:view' : 'network_devices:view');
      return html`<div class="health-row"><div class="toolbar"><strong>${o(type)}</strong><small>${subset.length || o(unavailable ? 'unavailableData' : !canView ? 'permission' : this.resourceOverview?.truncated ? 'incomplete' : this.search || this.engine ? 'noMatch' : 'noManaged')}</small></div>${subset.length ? html`<div class="health-bar" aria-hidden="true">${(['normal', 'abnormal', 'unavailable', 'unknown'] as RunState[]).map(state => html`<span class=${state} style=${`width:${subset.filter(item => runState(item) === state).length / subset.length * 100}%`}></span>`)}</div><div class="health-track">${(['normal', 'abnormal', 'unavailable', 'unknown'] as RunState[]).map(state => html`<button class=${`btn-ghost ${state}`} aria-pressed=${String(this.detailFilter === state && this.detailType === type)} @click=${() => this.selectDetail(state, type)}>${o(state === 'unavailable' && type === 'network_device' ? 'unreachable' : state)} ${subset.filter(item => runState(item) === state).length}</button>`)}</div>` : nothing}</div>`;
    })}<section class="collection-summary" aria-label=${o('collection')}><h2>${o('collection')}</h2><p class="metadata">${o('freshCoverage')} <strong>${s.fresh}/${s.total}</strong> · ${o('collectionRisk')} <strong>${s.collection}</strong></p><details><summary>${o('expandGaps')}</summary><div class="metadata">${['fresh', 'stale', 'missing', 'failed'].map(state => html`<span>${o(state)} ${items.filter(item => freshness(item) === state).length}</span>`)}<span>${o('qualityGap')} ${items.filter(item => item.quality !== 'good').length}</span></div></details></section>
      ${this.renderRelations(items)}</app-card>`;
  }
  private renderRelations(items: OverviewItem[]) {
    const selected = items.find(item => resourceKey(item.resource) === this.selectedRisk && isRisk(item));
    return html`<section id="risk-relations" class="relation-summary" aria-live="polite" aria-label=${o('relations')}>${!selected ? html`<small>${o('selectRisk')}</small>` : html`<h2>${o('relations')}</h2><p class="relation-source"><strong>${this.label(selected)}</strong></p>${selected.gaps.includes('RELATIONS_UNAVAILABLE') ? html`<p role="status">${o('relationUnavailable')}</p>` : !selected.impactScope.length ? html`<p>${o('noRelations')}</p>` : selected.impactScope.map(ref => {
    ...
    })}<p class="metadata">${o('relationHint')}</p><a href=${this.detailHref(selected.resource, true)} @click=${this.savePosition}>${o('evidence')}</a>`}</section>`;
  }
  ...
  private async loadTrend() {
    ...
    if (this.resourceScope !== 'instance' || !this._visibleResourceItems().length) { this.capacityTrend = null; this.trendLoading = false; return; }
    const id = this.selectedInstanceId;
    const params = new URLSearchParams(id ? { instance_id: String(id) } : { instance_ids: this._visibleResourceItems().map(item => item.resource.id).sort((a, b) => a - b).join(',') });
  ...
  private renderDatabase(items: OverviewItem[]) {
    ...
    const colors = engines.map((_, index) => `var(--overview-series-${index % 6})`);
    let angle = 0;
    const slices = engines.map(({ count }, index) => { const start = angle; angle += count / items.length * 360; return `${colors[index]} ${start}deg ${angle}deg`; });
    const last = this.capacityTrend?.trend.at(-1);
    const expected = this.selectedInstanceId ? 1 : items.length;
    const sampled = last?.instance_count;
    return html`<div class="database-panels"><app-card class="engine-panel"><h2 slot="header">${o('dbTypes')}</h2><div class="engine-distribution"><div class="engine-pie" role="img" aria-label=${engines.map(({ name, count }) => `${name}: ${count} (${Math.round(count / items.length * 100)}%)`).join(', ')} style=${`background: conic-gradient(${slices.join(',')})`}></div><div class="engine-legend">${engines.map(({ name, count }, index) => html`<button class="btn-ghost engine-key" aria-pressed=${String(this.engine === name)} @click=${() => { this.engine = this.engine === name ? '' : name; this.page = 1; this.normalizeSelection(); this.saveUrl(); void this.loadTrend(); }}><span class="series-dot" style=${`background:${colors[index]}`}></span><span>${name}</span><strong>${count}</strong><small>${Math.round(count / items.length * 100)}%</small></button>`)}</div></div><small>${o('instanceRatio')} · ${items.length}</small></app-card>
      <app-card class="capacity-panel"><div slot="header" class="toolbar"><h2>${o('capacity')}</h2><span class="metadata">${last ? html`${sampled !== expected ? o('collectedSize') + ' ' : ''}${this._formatBytes(last.total_size_gb)}` : '—'}</span></div><div class="filters capacity-controls"><app-form-field label=${o('chooseInstance')}><select aria-label=${o('chooseInstance')} .value=${String(this.selectedInstanceId ?? '')} @change=${(event: Event) => { this.selectedInstanceId = Number((event.target as HTMLSelectElement).value) || null; this.saveUrl(); void this.loadTrend(); }}><option value="" .selected=${live(this.selectedInstanceId === null)}>${o('scopeTotal')} (${items.length})</option>${items.map(item => html`<option value=${item.resource.id} .selected=${live(item.resource.id === this.selectedInstanceId)}>${this.label(item)}</option>`)}</select></app-form-field><div class="actions">${[24, 168, 720].map((hours, index) => html`<button class=${!this.customDates && this.selectedHours === hours ? 'btn-primary' : 'btn'} @click=${() => { this.customDates = false; this.selectedHours = hours; this.saveUrl(); void this.loadTrend(); }}>${['24h', '7d', '30d'][index]}</button>`)}<button class="btn" aria-expanded=${String(this.customDates)} @click=${() => { this.customDates = !this.customDates; }}>${o('custom')}</button></div></div>
      ...
      ${this.trendLoading && !this.capacityTrend ? html`<div class="skeleton large" aria-label=${o('refreshing')}></div>` : this.capacityTrend?.trend.length ? html`<div class="trend-chart-container" role="img" aria-label=${`${o('capacity')} GB`}></div><p class="metadata">${o('sampleCoverage')}: ${sampled ?? '—'}/${expected} · ${o('observed')}: ${this.time(last?.observed_at ?? last?.time ?? null)}${sampled !== expected ? html` · ${o('partialCapacity')}` : nothing}</p>` : html`<app-empty-state title=${o('noCapacity')}></app-empty-state>`}<details class="chart-notes"><summary>${o('dataNotes')}</summary><p class="metadata">${o('capacityGaps')} · ${o('dataTime')}: ${this.time(this.trendTime)}</p></details></app-card></div>`;
  }
  ...
  override updated(changes: Map<string, unknown>) {
    ...
    if (!container) this.disposeChart();
      ...
      for (const point of this.capacityTrend?.trend ?? []) {
        ...
        if (previous && previousCount !== point.instance_count) points.push([point.time, null]);
        points.push([point.time, point.total_size_gb]);
        previousCount = point.instance_count;
      }
      this.chart.setOption({ animation: false, textStyle: { color: text }, tooltip: { trigger: 'axis' }, grid: { left: 55, right: 20, top: 30, bottom: 40 }, xAxis: { type: 'time', axisLabel: { color: text } }, yAxis: { type: 'value', name: 'GB', axisLabel: { color: text } }, series: [{ type: 'line', name: o('capacity'), data: points, connectNulls: false, smooth: false, showSymbol: true, symbolSize: 5, lineStyle: { color: accent }, itemStyle: { color: accent } }] });
  ...
  override render() {
    ...
    return html`<div class="dashboard-grid"><header class="toolbar"><h1>${o('title')}</h1><div class="metadata"><time title=${this.resourceOverview?.collectedAt ?? ''}>${o('dataTime')}: ${this.time(this.resourceOverview?.collectedAt ?? null)}</time><button class="btn" .disabled=${this.loading} @click=${this.loadDashboardData}><span aria-hidden="true">${icons['refresh-cw']}</span>${o(this.loading ? 'refreshing' : 'refresh')}</button></div></header>
      <div class="scope-toolbar"><div class="scope-switch" role="tablist" aria-label=${o('title')}>${(['all', ...resourceTypes] as Scope[]).map((scope, index) => html`<button id=${`scope-${scope}`} class="btn" role="tab" aria-selected=${String(this.resourceScope === scope)} aria-controls="overview-panel" tabindex=${this.resourceScope === scope ? '0' : '-1'} @keydown=${(event: KeyboardEvent) => this.keydownTab(event, index)} @click=${() => this.switchScope(scope)}><span aria-hidden="true">${this.icon(scope)}</span>${o(scope)}</button>`)}</div>
      <div class="filters scope-filters"><app-form-field label=${o('scopeSearch')}><input type="search" placeholder=${o('searchHint')} aria-label=${o('scopeSearch')} .value=${this.search} @input=${(event: Event) => this.changeScopeFilter(event, 'search')}></app-form-field>${this.resourceScope === 'instance' ? html`<app-form-field label=${o('engine')}><select aria-label=${o('engine')} .value=${this.engine} @change=${(event: Event) => this.changeScopeFilter(event, 'engine')}><option value="">${o('allEngines')}</option>${engines.map(engine => html`<option value=${engine} .selected=${live(engine === this.engine)}>${engine} (${scopeInventory.filter(item => String(item.attributes?.dbType ?? 'unknown') === engine).length})</option>`)}</select></app-form-field>` : nothing}${this.search || this.engine ? html`<button class="btn-ghost" @click=${this.clearFilters}>${o('clear')}</button>` : nothing}</div></div>
      ...
      ${!this.resourceOverview ? this.loading ? html`<div class="dashboard__stat-cards">${[0, 1, 2, 3].map(() => html`<div class="skeleton"></div>`)}</div><div class="skeleton large"></div>` : html`<app-empty-state title=${o('unavailableData')}></app-empty-state>` : !scopeInventory.length ? html`<app-card class="inventory-empty"><div class="empty-content"><app-empty-state icon=${this.resourceScope === 'network_device' ? 'network' : 'database'} title=${inventoryFailed ? o('unavailableData') : this.resourceOverview.truncated ? o('incomplete') : noPermission ? o('permission') : `${o('noManaged')} ${o(this.resourceScope === 'all' ? 'resources' : this.resourceScope)}`} description=${inventoryFailed || this.resourceOverview.truncated || noPermission ? '' : o(this.resourceScope === 'network_device' ? 'networkManageHint' : 'manageHint')}></app-empty-state>${!this.resourceOverview.truncated && !inventoryFailed && !noPermission && this.resourceScope !== 'all' && permissionMatches(this.permissions, this.resourceScope === 'network_device' ? 'network_devices:manage' : this.resourceScope === 'server' ? 'servers:manage' : 'instance:manage') ? html`<a class="btn-primary" href=${this.resourceScope === 'network_device' ? '/network-devices' : this.resourceScope === 'server' ? '/servers' : '/instances-db'}>${o('manage')}</a>` : nothing}</div></app-card>` : !items.length ? html`<app-empty-state title=${o('noMatch')}></app-empty-state><button class="btn" @click=${this.clearFilters}>${o('clear')}</button>` : html`
        ...
        ${this.renderKpis(items)}${this.renderDatabase(items)}<div class="dashboard__primary"><div class="main-panels">${this.renderRiskQueue(items)}${this.renderMetrics(items)}${this.renderBackups(items)}${this.renderDetails(items)}</div>${this.renderHealth(items)}</div>`}
      </section></div>`;
  }
}
