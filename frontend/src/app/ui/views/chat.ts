...
async function _fetchAgentGreeting(requestUpdate: () => void): Promise<void> {
  ...
  try {
    const res = await authFetch('/api/chat/greeting', {
...
/**
 * Back-link rendering for analysis results in chat messages.
 * Scans assistant message text for instance IDs and alert IDs,
 * renders clickable navigation buttons that dispatch slide-navigate CustomEvents.
 */
/**
 * Collect verified instance/alert IDs from ALL tool results across all groups.
 * This must run before the per-group render loop so back-links in assistant
 * groups can validate against tool results from different groups.
 */
function collectToolIdsFromAllGroups(chatItems: Array<ChatItem | MessageGroup>): {
      ...
      try {
        ...
        if (Array.isArray(d?.instances)) {
          for (const inst of d.instances) {
            if (typeof inst.id === 'number' && typeof inst.name === 'string' && inst.name) {
        ...
        if (Array.isArray(d?.alerts)) {
          for (const a of d.alerts) {
            if (typeof a.id === 'number') alertIds.add(a.id);
          }
        }
      } catch { /* not JSON, skip */ }
    }
...
function renderBackLinks(
  group: MessageGroup,
  toolInstanceIds: Set<number>,
  toolAlertIds: Set<number>,
): TemplateResult | typeof nothing {
  // Scan assistant text for 实例 #N / 告警 #N references
  // IDs are pre-computed from ALL groups, not just the current one
  const linkedInstanceIds = new Set<number>();
  const linkedAlertIds = new Set<number>();
  ...
  for (const entry of group.messages) {
    ...
    const instanceRegex = /实例\s*[#:：]?\s*(\d+)/g;
    let m: RegExpExecArray | null;
    while ((m = instanceRegex.exec(content)) !== null) {
      const id = parseInt(m[1], 10);
      // If no tool data available, link all references (fallback to original behavior)
      if (!hasToolData || toolInstanceIds.has(id)) {
        linkedInstanceIds.add(id);
      }
    }

    const alertRegex = /告警\s*[#:：]?\s*(\d+)/g;
    while ((m = alertRegex.exec(content)) !== null) {
      const id = parseInt(m[1], 10);
      if (!hasToolData || toolAlertIds.has(id)) {
        linkedAlertIds.add(id);
      }
    }
  }

  if (linkedInstanceIds.size === 0 && linkedAlertIds.size === 0) {
    return nothing;
  }

  const buttons: TemplateResult[] = [];

  for (const id of linkedInstanceIds) {
    buttons.push(html`
      <button
        type="button"
        class="chat-link-btn"
  ...
  for (const id of linkedAlertIds) {
    buttons.push(html`
      <button
        type="button"
        class="chat-link-btn"
...
function renderWelcomeState(props: ChatProps, requestUpdate: () => void): TemplateResult {
  const name = props.assistantName || "Assistant";

  // Initiate greeting fetch on first render
  if (_agentGreeting === null && !_agentGreetingPending) {
    _fetchAgentGreeting(requestUpdate);
  }
    ...
    <div class="agent-chat__welcome" style="--agent-color: var(--accent)">
      ...
      ${avatar
        ...
        : html`<div class="agent-chat__avatar agent-chat__avatar--logo">
            <img src=${logoUrl} alt="Slide" />
          </div>`}
      <h2>${name}</h2>
      ${_agentGreeting
        ? html`
            <div class="agent-chat__badges">
              <div class="agent-chat__badge agent-chat__badge--greeting">
                ${unsafeHTML(toSanitizedMarkdownHtml(_agentGreeting!))}
              </div>
            </div>`
        : html`
            <div class="agent-chat__badges">
...
export function renderChat(props: ChatProps) {
  ...
  // Show reasoning when enabled AND reasoning level is not "off", OR when
  // messages contain thinking blocks (DirectAdapter mode with reasoning models).
  const hasThinkingMessages = props.messages.some(m => {
    const content = (m as Record<string, unknown>).content;
    if (Array.isArray(content)) {
      return content.some((c: Record<string, unknown>) => c.type === 'thinking');
    }
    return false;
  });
      ...
      <div class="chat-thread-inner">
        ${props.loading
          ? html`
            ...
            `
          : nothing}
        ${isEmpty && !vs.searchOpen ? renderWelcomeState(props, requestUpdate) : nothing}
        ${isEmpty && vs.searchOpen
          ? html` <div class="agent-chat__empty">No matching messages</div> `
          : nothing}
        ${(() => {
          // Pre-compute tool-verified IDs from ALL groups so back-links
          // in assistant groups can validate against tool results from
          // different groups (they are always in separate groups).
          const toolIds = collectToolIdsFromAllGroups(chatItems);
          return repeat(
          chatItems,
          (item) => item.key,
          (item) => {
