...
function renderReplyPill(replyTarget: NormalizedMessage["replyTarget"]) {
  ...
  return html`
    <div class="chat-reply-pill">
      <span class="chat-reply-pill__icon">${icons['message-square']}</span>
...
function isLocalAssistantAttachmentSource(source: string): boolean {
  ...
  if (/^\/(?:__slide__|media)\//.test(trimmed)) {
    return false;
  }
  return (
...
function buildAssistantAttachmentUrl(
  ...
  return `${normalizedBasePath}/__slide__/assistant-media?${params.toString()}`;
}
...
function renderExpandButton(markdown: string, onOpenSidebar: (content: SidebarContent) => void) {
  return html`
    ...
    >
      <span class="chat-expand-btn__icon" aria-hidden="true">${icons['panel-right-open']}</span>
    </button>
  `;
}
