...
import { extractCanvasShortcodes } from "../../src/chat/canvas-render.ts";
import {
  isToolCallContentType,
  isToolResultContentType,
  resolveToolBlockArgs,
} from "../../src/chat/tool-content.ts";
import { mediaKindFromMime } from "../../src/media/constants.ts";
...
function expandTextContent(text: string): {
  ...
  // Handle stub return value (array) vs expected object shape
  const extractedText = Array.isArray(extracted) ? '' : (extracted.text ?? '');
  const extractedPreviews = Array.isArray(extracted) ? [] : (extracted.previews ?? []);
  ...
  for (const preview of extractedPreviews) {
