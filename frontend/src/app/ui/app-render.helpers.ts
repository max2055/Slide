...
import { countHiddenCronSessions, resolveSessionOptionGroups } from "./session-presentation.ts";
export { parseSessionKey, resolveSessionDisplayName, isCronSessionKey, resolveSessionOptionGroups } from "./session-presentation.ts";
export type { SessionKeyInfo } from "./session-presentation.ts";
...
export function resolveAssistantAttachmentAuthToken(
  ...
  return (
    normalizeOptionalString(state.settings.username) ?? normalizeOptionalString(state.password) ?? null
  );
}
