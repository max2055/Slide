...
const acpMocks = vi.hoisted(() => ({
  ...
  readAcpSessionEntry: vi.fn<(params: { sessionKey: string; cfg?: SlideConfig }) => unknown>(
    () => null,
  ),
...
const ttsMocks = vi.hoisted(() => ({
  ...
  resolveTtsConfig: vi.fn((_cfg: SlideConfig) => ({ mode: "final" })),
}));
...
vi.mock("../../tts/tts.js", () => ({
  ...
  resolveTtsConfig: (cfg: SlideConfig) => ttsMocks.resolveTtsConfig(cfg),
}));
...
vi.mock("./dispatch-acp-session.runtime.js", () => ({
  readAcpSessionEntry: (params: { sessionKey: string; cfg?: SlideConfig }) =>
    acpMocks.readAcpSessionEntry(params),
}));
vi.mock("../../tts/tts-config.js", () => ({
  normalizeTtsAutoMode: (value: unknown) => ttsMocks.normalizeTtsAutoMode(value),
  resolveConfiguredTtsMode: (cfg: SlideConfig) => ttsMocks.resolveTtsConfig(cfg).mode,
  shouldAttemptTtsPayload: () => true,
}));

export const noAbortResult = { handled: false, aborted: false } as const;
export const emptyConfig = {} as SlideConfig;
